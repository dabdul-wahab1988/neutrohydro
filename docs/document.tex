...
\begin{document}
	...
	\begin{itemize}
		...
		\item \textbf{Thermodynamic Validation}: Integrated \textbf{PHREEQC} engine for Saturation Index (SI) calculation and redox-aware speciation (Eh)
		\item \textbf{Redox Detection}: Explicitly solves for mass loss (e.g., Denitrification) using negative stoichiometry
		\item \textbf{Advanced Indices}: Integrated Simpson's Ratio (Revelle Coefficient), Freshening Ratio (FR), and Base Exchange Index (BEX) for precise salinity and process diagnosis
	\end{itemize}
	...
	\begin{lstlisting}[caption={Basic pipeline usage example}]
		...
		# 3. Fit the Model with PHREEQC Validation
		# X: predictor matrix, y: target variable
		# c_meq: ion concentrations for mineral solver
		feature_cols = ["Ca", "Mg", "Na", "K", "HCO3", "Cl", "SO4", "NO3"]
		X = df[feature_cols].values
		y = df["TDS"].values
		c_meq = df[feature_cols].values
	...
	\begin{lstlisting}[caption={NeutroHydroPipeline class definition}]
		...
		**kwargs : dict
		...
		def fit(self, X, y, feature_names=None, c_meq=None, pH=None, Eh=None, temp=25.0):
		"""
		...
		c_meq : Ion concentrations (meq/L)
		pH, Eh : Groundwater parameters for PHREEQC
