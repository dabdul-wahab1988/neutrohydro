...
\fancyhead[R]{\thepage}
\fancyfoot[C]{Neutrosophic Chemometrics for Groundwater Analysis}
...
\begin{document}
	...
	\begin{abstract}
		NeutroHydro implements a mathematically well-posed workflow for groundwater chemometrics in absolute concentration space. The framework combines neutrosophic logic with partial least squares regression to analyze groundwater chemistry, providing unambiguous attribution of prediction importance to baseline versus perturbation sources. This document presents the complete mathematical foundations, implementation details, and usage guidelines for the NeutroHydro framework.
	\end{abstract}
	...
	\begin{enumerate}
		\item Raw Ion Data
		\item Preprocessing (Robust centering/scaling)
		\item NDG Encoder (T, I, F triplets)
		\item PNPLS Regression (Augmented Hilbert space)
		\item NVIP (Channel-wise variable importance)
		\item NSR/$\pi_G$ (Baseline vs perturbation attribution)
		\item Mineral Inference (Stoichiometric inversion)
	\end{enumerate}
	...
	\begin{itemize}
		...
		\item \textbf{Redox Detection}: Explicitly solves for mass loss (e.g., Denitrification) using negative stoichiometry
		\item \textbf{Advanced Indices}: Integrated Simpson's Ratio (Standard \& Inverse) for precise salinity diagnosis (Seawater vs. Recharge)
	\end{itemize}
	...
	This section provides a practical introduction to using NeutroHydro for groundwater analysis.
	
	\subsection{Basic Workflow}
	
	The core of NeutroHydro is the \texttt{NeutroHydroPipeline} class. It handles preprocessing, encoding, model training, and mineral inversion in a single integrated workflow.
	
	\subsubsection{Preparing Your Data}
	
	Prepare a CSV file with your ion concentrations. The columns should match standard chemical symbols (e.g., \texttt{Ca}, \texttt{Mg}, \texttt{Na}, \texttt{HCO3}, \texttt{Cl}, \texttt{SO4}).
	...
	\begin{lstlisting}[caption={Basic pipeline usage example}]
		import pandas as pd
		...
		# 'target_ions' are the ions you want to model/predict
	...
	\subsection{Simpson's Ratio}
	
	Simpson's ratio distinguishes salinity mechanisms:
	
	\subsubsection{Standard Simpson's Ratio}
	\begin{equation}
		\text{SR}_{\text{std}} = \frac{\text{Cl}^- - 0.87 \times \text{Na}^+}{0.87 \times \text{SO}_4^{2-}}
	\end{equation}
	
	\textbf{Classification:}
	\begin{itemize}
		\item $\text{SR} < -1$: Fresh (Class 0)
		\item $-1 \leq \text{SR} < 1$: Slight (Class 1)
		\item $1 \leq \text{SR} < 2$: Moderate (Class 2)
		\item $\text{SR} \geq 2$: Severe (Class 3)
	\end{itemize}
	
	\subsubsection{Inverse Simpson's Ratio}
	\begin{equation}
		\text{SR}_{\text{inv}} = \frac{0.87 \times \text{SO}_4^{2-}}{\text{Cl}^- - 0.87 \times \text{Na}^+}
	\end{equation}
	
	\textbf{Mechanism:}
	\begin{itemize}
		\item $\text{SR}_{\text{inv}} > 1$: Recharge-driven
		\item $\text{SR}_{\text{inv}} < 0.5$: Intrusion-driven
	\end{itemize}
	...
	\subsubsection{Thermodynamic Blindness}
	
	\textbf{Issue:} The model is mass balance-based, not thermodynamic.
	
	\textbf{Implication:} It can propose minerals that are thermodynamically unfavorable.
	
	\textbf{Recommendation:} Validate results with PHREEQC or similar equilibrium codes.
	...
	\begin{lstlisting}[caption={NeutroHydroPipeline class definition}]
		...
		def __init__(self, target_ions: list[str], **kwargs):
		"""
		Initialize the pipeline.
		
		Parameters
		----------
		target_ions : list[str]
		List of ions to model (e.g., ['Ca', 'Mg', 'Na', ...])
		**kwargs : dict
		Additional parameters for encoder, model, etc.
		"""
		...
	...
	\begin{verbatim}
		@software{neutrohydro,
			title = {NeutroHydro: Neutrosophic Chemometrics 
